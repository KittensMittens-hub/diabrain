...
df <- df |> 
  select(
    ...
    `BMI 0`, `GFR0`, `NSE 0`, `S100 0`, `MMSE 0`, `HbA1c 0`,
...
df_long_cc <- df_long |> 
  # value ~ (Treatment * time) * (Age + Sex + log_DM_duration + BMI_0 + Hypertension 
  # + HbA1c_0 + GFR_0) + Stroke + Polineuropathy,
  drop_na(c(
    Age, Sex, BMI_0, GFR_0,
...
fit_met <- lm(
  ...
  Lchains_0 ~ Treatment * (Age + Sex + log_DM_duration + BMI_0 + Hypertension + HbA1c_0 + GFR_0) + Stroke + Polineuropathy,
  data = df
)
...
fit_gee_cc <- geepack::geeglm(
  value ~ (Treatment * time) * (Age + log_DM_duration + BMI_0 + HbA1c_0 + GFR_0)+ Stroke + Polineuropathy + Sex + Hypertension,
  id = ID,
  data = df_long_cc,
  family = "gaussian",
  corstr = "independence"
...
pred_cc |>
  ...
  scale_y_continuous(name = "Light chains concentrations", n.breaks = 8) +
  labs(
    title = 'Effects of SGLT2i and GLP1 on \nlight chains concentration'
  )
...
effect_cc |>
  ...
  labs(
    title = 'Comparison effects of SGLT2i and GLP1 on \nlight chains concentration'
  )
...
imp_wide <- mice::mice(
  ...
  formulas = list(
    ...
    "BMI_0" = BMI_0 ~ Treatment * (Age + Sex + Hypertension + HbA1c_0 + log_DM_duration + GFR_0 
                                   + Lchains_0 + Lchains_3 + Lchains_6 + Lchains_12) + Stroke + Polineuropathy,
    ...
    "Polineuropathy" = Polineuropathy ~ Treatment * (Age + Sex + BMI_0 + HbA1c_0 + log_DM_duration + Hypertension + GFR_0
                                                     + Lchains_0 + Lchains_3 + Lchains_6 + Lchains_12) + Stroke,
    "Stroke" = Stroke ~ Treatment * (Age + Sex + BMI_0 + HbA1c_0 + log_DM_duration + Hypertension + GFR_0
                                     + Lchains_0 + Lchains_3 + Lchains_6 + Lchains_12) + Polineuropathy,
    
    "Lchains_0" = Lchains_0 ~ Treatment * (Age + Sex + log_DM_duration + BMI_0 + Hypertension + HbA1c_0 + GFR_0
                                           + Lchains_3 + Lchains_6 + Lchains_12) + Stroke + Polineuropathy,
    "Lchains_3" = Lchains_3 ~ Treatment * (Age + Sex + log_DM_duration + BMI_0 + Hypertension + HbA1c_0 + GFR_0
                                           + Lchains_0 + Lchains_6 + Lchains_12) + Stroke + Polineuropathy,
    "Lchains_6" = Lchains_6 ~ Treatment * (Age + Sex + log_DM_duration + BMI_0 + Hypertension + HbA1c_0 + GFR_0
                                           + Lchains_0 + Lchains_3 + Lchains_12) + Stroke + Polineuropathy,
    "Lchains_12" = Lchains_12 ~ Treatment * (Age + Sex + log_DM_duration + BMI_0 + Hypertension + HbA1c_0 + GFR_0
                                             + Lchains_0 + Lchains_3 + Lchains_6) + Stroke + Polineuropathy
  ),
  m = m, maxit = it, seed = seed, 
  printFlag = FALSE
)
...
fit_gee_mice <- map(seq_len(m), function(i) {
  d <- imp_long[[i]]
  geepack::geeglm(
    # value ~ (Treatment * time) * (Age + Sex + Hypertension + BMI_0 + HbA1c_0)
...
pred_mice |>
  ...
  geom_point(position = pd, size = 2) +
  scale_color_manual(values = group_cols) +
  scale_y_continuous(name = "Light chains concentration", n.breaks = 8) +
  labs(
    title = 'Effects of SGLT2i and GLP1 on \nlight chains concentration',
