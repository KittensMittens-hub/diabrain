...
df <- df |> 
  select(
    ...
    Hypertension, Polineuropathy, Stroke
  ) |> 
  rename(`GFR 0` = `GFR0`) |> 
...
fit_gee_cc <- geepack::geeglm(
  value ~ (Treatment * time) * (Age + Sex + log_DM_duration + BMI_0 + 
                                Hypertension + HbA1c_0 + GFR_0) + Stroke + Polineuropathy + Stroke,
  id = ID,
  data = df_long_cc,
  family = "gaussian",
  corstr = "independence"
...
pred_cc |>
  ...
  scale_y_continuous(name = "NSE", n.breaks = 8) 
...
imp_wide <- mice::mice(
  ...
  formulas = list(
    ...
    "NSE_0" = NSE_0 ~ Treatment * (Age + Sex + log_DM_duration + BMI_0 + Hypertension + HbA1c_0 + GFR_0 + NSE_3 + NSE_6 + NSE_12),
    "NSE_3" = NSE_3 ~ Treatment * (Age + Sex + log_DM_duration + BMI_0 + Hypertension + HbA1c_0 + GFR_0 + NSE_0 + NSE_6 + NSE_12),
    "NSE_6" = NSE_6 ~ Treatment * (Age + Sex + log_DM_duration + BMI_0 + Hypertension + HbA1c_0 + GFR_0 + NSE_0 + NSE_3 + NSE_12),
    "NSE_12" = NSE_12 ~ Treatment * (Age + Sex + log_DM_duration + BMI_0 + Hypertension + HbA1c_0 + GFR_0 + NSE_0 + NSE_3 + NSE_6)
  ),
  m = m, maxit = it, seed = seed, 
  printFlag = FALSE
)
