...
=======
  ...
  'Mean' = ~round(mean(.x, na.rm = T), 1) %>% as.character(),
  'Median' = ~round(median(.x, na.rm = T),1) %>% as.character(),
  'Sd' = ~sd(.x, na.rm = T) %>% round(2) %>% as.character(),
  'Q1-Q3' = ~paste0(round(quantile(.x, .25, na.rm = TRUE), 1), ' - ',
                             round(quantile(.x, .75, na.rm = TRUE),1)),
  'Min-Max' = ~paste0(round(min(.x, na.rm = T),1), ' - ', round(max(.x, na.rm = T),1)) %>% as.character()
  )
>>>>>>> 01d7f92dd3fbfe94bd92c747e91b85797d1ce46a
```
...
allvariablstatic_cat <- function(selection) {
  merged_data %>%
  select(all_of({{selection}})) %>% 
  summarise( 
    across(everything(), statistic2)
    ) %>% 
  pivot_longer(everything(), names_to = c('Variable', 'Statistic'), names_sep = '_', values_to = 'Valuetotal') }
...
#Функция создания DF по группам лечения (качественные признаки)
variablstatic_cat <- function(tratgr, selection) {
  merged_data %>%
  filter(Treatment == {{tratgr}}) %>% 
  select(Treatment, all_of({{selection}})) %>% 
  summarise( 
    across(!Treatment, statistic2)
    ) %>% 
  pivot_longer(everything(), names_to = c('Variable', 'Statistic'), names_sep = '_', values_to = 'Valueyes')}
...
as_flextable(demogranalisis_col) %>% 
  ...
  set_header_labels(Valuetotal = 'All', ...4 = 'Metformin', ...5 = 'GLP1', ...6 = 'SGLT2i') %>%
