...
```{r}
ggplot(merged_data, aes(y = `DM duration`, x = Treatment.x)) +
  geom_boxplot(width = 0.5, position = position_dodge(width = 0.6), alpha = 0.6) +
  labs(
    x = "Группа терапии",
...
NSE_long <- merged_data %>%
  select(`NSE 0`, `NSE 3`, `NSE 6`, `NSE 12`) %>% 
...
ggplot(NSE_long, aes(x = month, y = NSE)) +
  ...
  labs(
    x = "Месяц наблюдения",
    y = "Нейронспецифическая енолаза, нг/мл",
    title = "Значения показателя Нейронспецифическая енолаза по визитам"
  ) +
  theme_bw()+
  theme(plot.title = element_text(hjust = 0.5))
```
...
Lchains_long <- merged_data %>%
  select(`Lchains 0`, `Lchains 3`, `Lchains 6`, `Lchains 12`) %>% 
...
ggplot(Lchains_long, aes(x = month, y = Lchains)) +
  ...
  labs(
    x = "Месяц наблюдения",
    y = "Легкие цепи нейрофиламента, нг/мл",
...
S100_long <- merged_data %>%
  select(`S100 0`, `S100 3`, `S100 6`, `S100 12`) %>% 
