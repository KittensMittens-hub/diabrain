...
```{r}
moca_long <- merged_data %>%
  select(ID, Treatment, Age, Sex, `DM duration`,
         `MOCA 0`, `MOCA 3`, `MOCA 6`, `MOCA 9`, `MOCA 12`) %>%
  pivot_longer(
    cols = starts_with("MOCA"),
    names_to = "visit",
    values_to = "MOCA"
  ) %>%
  mutate(
    ...
    month_f = factor(month, levels = c(0, 3, 6, 9, 12)),
    Treatment = factor(Treatment),
    Sex = factor(Sex),
    MOCA = as.numeric(MOCA),
    DMdur = as.numeric(`DM duration`)
  )
...
moca_2g <- moca_mod %>%
  filter(Treatment %in% c("GLP1", "SGLT2i")) %>%
  droplevels()

with(moca_2g, table(month_f, Treatment))
summary(table(moca_2g$ID))

m_moca_2g <- geeglm(
  MOCA ~ month_f * Treatment + Age + Sex + DMdur,
  id = ID,
  waves = month,
  data = moca_2g,
  family = gaussian(),
...
В модели с взаимодействием время\*группа терапии, включающей пациентов, получавших GLP-1 и SGLT2i, отмечалось статистически значимое увеличение показателей MOCA во времени в обеих группах. По сравнению с исходным визитом, к 6-му, 9-му и 12-му месяцам наблюдения баллы MOCA достоверно возрастали (p \< 0.01), что свидетельствует об улучшении когнитивной функции в процессе наблюдения. При этом статистически значимого взаимодействия между временем и группой терапии выявлено не было, что указывает на сопоставимую динамику изменений MOCA в группах GLP-1 и SGLT2i. Различия между группами на исходном визите также не достигали статистической значимости. Возраст пациентов оказывал независимое отрицательное влияние на уровень когнитивной функции, тогда как пол и длительность сахарного диабета не были ассоциированы с показателями MOCA. Внутрипациентская корреляция повторных измерений была умеренной (AR-1 α = 0.57), что подтверждает корректность использования продольной модели GEE.

```{r}
library(emmeans)

emm_moca <- emmeans(m_moca_2g, ~ Treatment | month_f)
emm_moca
pairs(emm_moca, adjust = "bonferroni")

emm_df <- as.data.frame(emm_moca)

ggplot(emm_df, aes(month_f, emmean, group = Treatment, color = Treatment)) +
  geom_line(size = 1) +
  geom_point(size = 3) +
  ...
  labs(
    x = "Месяц наблюдения",
    y = "MOCA, баллы",
    title = "Модельная динамика MOCA: GLP1 vs SGLT2i"
  ) +
  theme_bw()
```

На основе обобщенной оценочной модели были рассчитаны скорректированные средние значения MOCA по визитам для групп GLP-1 и SGLT2i. В обеих группах наблюдалось постепенное увеличение баллов MOCA в течение периода наблюдения. На исходном визите статистически значимых различий между группами выявлено не было (p = 0.58). Аналогично, различия между группами на 3-м, 9-м и 12-м месяцах наблюдения не достигали статистической значимости (p \> 0.1).

На 6-м месяце наблюдения было выявлено статистически значимое различие между группами: в группе SGLT2i средний балл MOCA был выше по сравнению с группой GLP-1 (разница 1.37 балла, p = 0.014). Однако данное различие носило транзиторный характер и не сохранялось на последующих визитах. В целом, полученные данные свидетельствуют о сопоставимой динамике когнитивной функции в группах GLP-1 и SGLT2i, что согласуется с отсутствием статистически значимого взаимодействия время\*группа терапии в основной модели.
...
nse_long <- merged_data %>%
  select(ID, Treatment, Age, Sex, `DM duration`,
         `NSE 0`, `NSE 3`, `NSE 6`, `NSE 12`) %>% # у NSE нет 9 месяца
  pivot_longer(
    cols = starts_with("NSE"),
    names_to = "visit",
    values_to = "NSE"
  ) %>%
  mutate(
...
with(nse_2g, table(month_f, Treatment))
summary(table(nse_2g$ID))

m_nse_2g <- geeglm(
  NSE ~ month_f * Treatment + Age + Sex + DMdur,
  id = ID,
  waves = month,
  data = nse_2g,
  family = gaussian(),
...
В модели с взаимодействием время\*группа терапии, включающей пациентов, получавших GLP-1 и SGLT2i, статистически значимого взаимодействия между временем наблюдения и типом терапии выявлено не было (p \> 0.2 для всех interaction-терминов). Это указывает на отсутствие различий в траектории изменения уровня нейронспецифической енолазы между группами терапии.

В группе GLP-1 отмечалась тенденция к снижению уровня NSE к 12-му месяцу наблюдения по сравнению с исходным визитом (-1.85 нг/мл, p = 0.021). В группе SGLT2i данное снижение не отличалось статистически значимо от динамики в группе GLP-1, что подтверждается отсутствием значимого взаимодействия «время × терапия» на всех визитах. Возраст, пол и длительность сахарного диабета не были статистически значимо ассоциированы с уровнем NSE. Внутрипациентская корреляция повторных измерений была умеренной (AR-1 α = 0.62), что подтверждает корректность применения продольной модели GEE.

### Lchains

```{r}
lch_long <- merged_data %>%
  select(ID, Treatment, Age, Sex, `DM duration`,
         `Lchains 0`, `Lchains 3`, `Lchains 6`, `Lchains 12`) %>%
  pivot_longer(
    cols = starts_with("Lchains"),
    names_to = "visit",
    values_to = "Lchains"
  ) %>%
  mutate(
    ...
    Treatment = factor(Treatment),
    Sex = factor(Sex),
    Lchains = as.numeric(Lchains),
    DMdur = as.numeric(`DM duration`)
  )

lch_mod <- lch_long %>%
  filter(!is.na(Lchains)) %>%
  arrange(ID, month)
...
with(lch_2g, table(month_f, Treatment))
summary(table(lch_2g$ID))

m_lch_2g <- geeglm(
  Lchains ~ month_f * Treatment + Age + Sex + DMdur,
  id = ID,
  waves = month,
  data = lch_2g,
  family = gaussian(),
...
lch_2g <- lch_2g %>%
  mutate(logLchains = log1p(Lchains))

m_log_lch_2g <- geeglm(
  logLchains ~ month_f * Treatment + Age + Sex + DMdur,
  id = ID,
  waves = month,
  data = lch_2g,
  family = gaussian(),
...
summary(m_log_lch_2g)
```

В модели с взаимодействием время\*группа терапии с использованием лог-трансформированного уровня легких цепей нейрофиламента устойчивых различий в динамике показателя между группами GLP-1 и SGLT2i выявлено не было. На 3-м месяце наблюдения отмечалось кратковременное различие в изменении уровня Lchains между группами, однако данный эффект не сохранялся на последующих визитах. В целом траектории изменения Lchains в группах терапии были сопоставимы. Мужской пол ассоциировался с более высоким уровнем Lchains, тогда как возраст и длительность сахарного диабета статистически значимого влияния не оказывали.
