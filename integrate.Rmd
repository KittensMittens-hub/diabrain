...
library(lme4)
library(emmeans)
model_scales <- lmer(MOCA ~ Treatment*time + Age + Sex + HbA1c + 
                       Hypertension + (1 | ID),
                     data = data_long_f)
emmeans_treatment <- emmeans(model_scales, specs = pairwise ~ Treatment)
summary(emmeans_treatment$emmeans)
summary(emmeans_treatment$contrasts) 
```
# Модель для MMSE 
```{r}
